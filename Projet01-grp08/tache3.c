...
{
	int *tab;
	int taille; // repr é sente la taille r é elle de l ’ ensemble
};
...
int main(int argc, char const *argv[])
{
	MonEnsemble A,B,C;
	int j,k,i,tmp;
	printf("\nImplementation des fonctions en supposant que les tableaux sont non tries \n\n");
/***************************************************************************************************/
...
/***************************************************************************************************/
	...
	scanf("%d",&A.taille);
	A.tab=malloc(A.taille*sizeof(int));
	init(&A);
...
/***************************************************************************************************/
	...
	scanf("%d",&B.taille);
	B.tab=malloc(B.taille*sizeof(int));
	init(&B);
	printf("Entrez les donnees de l'ensemble B\n");
	for(i=0;i<B.taille; ++i)
		scanf("%d",&B.tab[i]);
	C.tab=malloc((A.taille+B.taille)*sizeof(int));
	init(&C);
	printf("\n A U B en cours...\n");
    unionn(&A,&B,&C);
    suppression(&C);
